...
const PRESETS: DesignPreset[] = [
  ...
  preset(
    ...
    ['client', 'gateway', 'auth', 'app', 'queue', 'sql', 'nosql'],
    ['client', 'gateway', 'app', 'sql'],
    'Split the monolith',
    'Orders flow through a gateway into auth and dedicated services, with a queue decoupling fulfillment from the durable order store.',
    'Add Auth plus a second App, put Queue between App and SQL, keep Gateway at the edge, then Run.',
    'Microservices-shaped — edge policy, owned services, async fulfillment.',
    (s) => kindsPresent(s, ['gateway', 'auth']) && countKind(s, 'app') >= 2 && hasPath(s, ['app'], ['queue']) && hasPath(s, ['queue'], SINK_KINDS),
  ),
  preset(
    'websocket-chat',
    ['client', 'gateway', 'app', 'nosql', 'queue', 'cache'],
    ['client', 'gateway', 'app', 'nosql'],
    'Push over WebSockets',
    'Chat terminates persistent connections at the gateway; app servers fan out through a queue into fast NoSQL so delivery never blocks the socket.',
    'Wire Client → Gateway → App, add Queue plus NoSQL behind App, then Run.',
    'Realtime push — sockets stay open, delivery flows async.',
    (s) => kindsPresent(s, ['gateway', 'app']) && hasPath(s, ['app'], ['queue']) && hasPath(s, ['queue'], ['nosql']) && chainOk(s),
  ),
  preset(
    'cdn-video-streaming',
    ['client', 'dns', 'cdn', 'app', 'storage', 'sql'],
    ['client', 'cdn', 'storage'],
    'Serve video at the edge',
    'Bytes come from cache and object storage, never the origin. The app tier only serves metadata while the CDN absorbs the flood.',
    'Wire Client → CDN → Object Storage, keep App → SQL for metadata, then Run.',
    'Edge-served — origin sees metadata, CDN takes the bytes.',
    (s) => kindsPresent(s, ['cdn', 'storage']) && hasPath(s, ['client'], ['cdn']) && chainOk(s),
  ),
  preset(
    'search-autocomplete',
    ['client', 'lb', 'app', 'search', 'cache', 'sql'],
    ['client', 'app', 'search'],
    'Answer as they type',
    'Every keystroke is a query. Cache absorbs repeats and a dedicated search index answers in milliseconds while SQL stays the source of truth.',
    'Add Cache between App and Search, put LB in front of 2× App, then Run.',
    'Instant search — cached, indexed, off the primary.',
    (s) => hasPath(s, ['app'], ['cache']) && hasPath(s, ['cache'], ['search']) && chainOk(s),
  ),
  preset(
    'free',
    FULL,
...
const SLOS: Record<string, RunSLO> = {
  ...
  'microservices-orders': { p99LtMs: 400, minCompleted: 100 },
  'websocket-chat': { errLtPct: 5, minCompleted: 100 },
  'cdn-video-streaming': { p99LtMs: 200, minCompleted: 100 },
  'search-autocomplete': { p99LtMs: 250, minCompleted: 100 },
